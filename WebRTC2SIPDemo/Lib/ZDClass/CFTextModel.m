...
- (void)setupEmoticonMapper:(NSDictionary *)emoticonMapper
{
    ...
    if (_mapper.count == 0) {
        ...
        for (NSUInteger i = 0, max = allKeys.count; i < max; i++) {
            ...
            //NSDictionary *emoji_dic = _mapper[key];
            NSMutableString *one = [key mutableCopy];
...
- (BOOL)parseText:(NSMutableAttributedString *)text {
    ...
    for (NSUInteger i = 0, max = matches.count; i < max; i++)
    {
        ...
        NSDictionary *emjoiDic = mapper[subStr];
        NSString *emoticonStr = emjoiDic[@"gif"];
...
- (void)setContentString:(NSString *)contentString
{
    ...
    NSString *path = [[NSBundle mainBundle] pathForResource:@"IM_Emojis" ofType:@"plist"];
