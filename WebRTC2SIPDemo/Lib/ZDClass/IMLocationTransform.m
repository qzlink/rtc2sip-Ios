//
//  IMLocationTransform.m
//  MOXIN
//
//  Created by DongDong on 2019/7/1.
//  Copyright © 2019年 mengya. All rights reserved.
//

#import "IMLocationTransform.h"
...
@implementation IMLocationTransform
...
- (id)transformFromGPSToGD {
    CLLocationCoordinate2D coor = [IMLocationTransform transformFromWGSToGCJ:CLLocationCoordinate2DMake(self.latitude, self.longitude)];
    return [[IMLocationTransform alloc] initWithLatitude:coor.latitude andLongitude:coor.longitude];
}

- (id)transformFromGDToBD {
    CLLocationCoordinate2D coor = [IMLocationTransform transformFromGCJToBaidu:CLLocationCoordinate2DMake(self.latitude, self.longitude)];
    return [[IMLocationTransform alloc] initWithLatitude:coor.latitude andLongitude:coor.longitude];
}

- (id)transformFromBDToGD {
    CLLocationCoordinate2D coor = [IMLocationTransform transformFromBaiduToGCJ:CLLocationCoordinate2DMake(self.latitude, self.longitude)];
    return [[IMLocationTransform alloc] initWithLatitude:coor.latitude andLongitude:coor.longitude];
}

- (id)transformFromGDToGPS {
    CLLocationCoordinate2D coor = [IMLocationTransform transformFromGCJToWGS:CLLocationCoordinate2DMake(self.latitude, self.longitude)];
    return [[IMLocationTransform alloc] initWithLatitude:coor.latitude andLongitude:coor.longitude];
}

- (id)transformFromBDToGPS {
    //先把百度转化为高德
    CLLocationCoordinate2D start_coor = [IMLocationTransform transformFromBaiduToGCJ:CLLocationCoordinate2DMake(self.latitude, self.longitude)];
    CLLocationCoordinate2D end_coor = [IMLocationTransform transformFromGCJToWGS:CLLocationCoordinate2DMake(start_coor.latitude, start_coor.longitude)];
    return [[IMLocationTransform alloc] initWithLatitude:end_coor.latitude andLongitude:end_coor.longitude];
}
